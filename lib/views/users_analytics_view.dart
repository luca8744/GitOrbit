...
class _UsersAnalyticsViewState extends State<UsersAnalyticsView> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final allUsers = await _client.getUsers();
      ...
      // Filter out users with 0 activities
      final activeUsers = allUsers.where((user) => (weeklyCounts[user.id] ?? 0) > 0).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Active Team')),
      body: RefreshIndicator(
        onRefresh: _loadData,
        child: _users.isEmpty
            ? const Center(child: Text('No active users in the last 7 days.'))
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (user.lastActivityOn != null) {
                    ...
                    lastActive = '${localDate.year}-${localDate.month.toString().padLeft(2, '0')}-${localDate.day.toString().padLeft(2, '0')}';
                  }
